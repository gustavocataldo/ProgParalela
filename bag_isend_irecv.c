...
void validador_do_isend(const void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm, MPI_Request *request, MPI_Status *status, int *flag) {
    double t_i, t_f;
    t_i = MPI_Wtime();
...
/* Envia pedaços com TAMANHO números para cada processo */
    if (meu_ranque == 0) { 
        for (dest=1, inicio=3; dest < num_procs && inicio < n; dest++, inicio += TAMANHO) {
            MPI_Isend(&inicio, 1, MPI_INT, dest, tag, MPI_COMM_WORLD, &request_send);
            validador_do_isend(&inicio, 1, MPI_INT, dest, tag, MPI_COMM_WORLD, &request_send, &estado, &flag);
        }
/* Fica recebendo as contagens parciais de cada processo */
        while (stop < (num_procs-1)) {
		    MPI_Irecv(&cont, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &request_recv);
            MPI_Wait(&request_recv, &estado);
            total += cont;
...
/* Cada processo escravo recebe o início do espaço de busca */
        while (estado.MPI_TAG != 99) {
            MPI_Irecv(&inicio, 1, MPI_INT, raiz, MPI_ANY_TAG, MPI_COMM_WORLD, &request_recv);
            MPI_Wait(&request_recv, &estado);
...
/* Envia a contagem parcial para o processo mestre */
                MPI_Isend(&cont, 1, MPI_INT, raiz, tag, MPI_COMM_WORLD, &request_send);
                validador_do_isend(&cont, 1, MPI_INT, raiz, tag, MPI_COMM_WORLD, &request_send, &estado, &flag);
            } 
        } 
