...
/* Envia pedaços com TAMANHO números para cada processo */
    if (meu_ranque == 0) { 
        for (dest=1, inicio=3; dest < num_procs && inicio < n; dest++, inicio += TAMANHO) {
            ...
            validador_do_isend(&inicio, 1, MPI_INT, dest, tag, MPI_COMM_WORLD, &request, &estado, &flag);
        }
...
/* Envia a contagem parcial para o processo mestre */
                ...
                validador_do_isend(&cont, 1, MPI_INT, raiz, tag, MPI_COMM_WORLD, &request, &estado, &flag);
            } 
        } 
